...
def tick_prices(bid, ask, now_t=None):
    ...
    if TREND_FILTER_ENABLED and HEDGE_CLOSE_ENABLED and not _prev_hedge_state:
        ...
        for level, pair in by_level.items():
            ...
            p = a if h is b else b  # the paired trade (same level, opp side)
            # Close wrong-side hedge at market
            ...
            # Also close the paired trade to prevent orphan expiry
            if p in trades:
                pnl2 = (p.price - bid) / PIP * PIP_VALUE * lot_size if p.side == 'sell' else (bid - p.price) / PIP * PIP_VALUE * lot_size if p.side == 'buy' else 0
                balance += pnl2
                hit_log.append({'t': time.strftime('%H:%M:%S', time.localtime(now_ts)), 'side': 'CLEANUP-PAIR-' + p.side.upper(), 'entry': round(p.price, 2), 'exit': round(bid if p.side == 'sell' else ask, 2), 'price': round(bid if p.side == 'sell' else ask, 2), 'pnl': round(pnl2, 2), 'dur': _fmt_dur(p.entry_time, now_ts), 'entry_time': time.strftime('%Y-%m-%d %H:%M', time.localtime(p.entry_time)), 'exit_time': time.strftime('%Y-%m-%d %H:%M', time.localtime(now_ts))})
                _remove_safe(trades, p)
                if p.id in trade_age:
                    del trade_age[p.id]
            if h.id in trade_age:
                del trade_age[h.id]
            # Don't replenish either — let grid orders at adjacent levels handle fresh entries
            ...
            if bid >= t.tp:
                ...
                for h in trades[:]:
                    if h.side == 'sell' and h.level_idx == t.level_idx:
                        loss = (h.price - t.tp) / PIP * PIP_VALUE * lot_size
                        balance += loss
            ...
            if ask <= t.tp:
                ...
                for h in trades[:]:
                    if h.side == 'buy' and h.level_idx == t.level_idx:
                        loss = (t.tp - h.price) / PIP * PIP_VALUE * lot_size
                        balance += loss
