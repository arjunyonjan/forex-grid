...
def _write_trade_log(side, entry, exit_p, pnl, gross, costs, reason):
...
def account_summary():
    ...
    rv = dict(
        ...
        total_pnl=round(total_pnl, 2), win_pnl=win_pnl,
        wins=sum(1 for h in hit_log if h.get("pnl",0)>0),
        losses=sum(1 for h in hit_log if h.get("pnl",0)<0),
        loss_pnl=loss_pnl, return_pct=round(total_pnl / start_balance * 100, 2),
...
def _fmt_dur(s, now=None):
    ...
    return "%dm%ds" % (int(d/60), int(d%60))
...
def tick_prices(bid, ask, grid, now=None):
    global orders, trades, balance, daily_trade_count, total_trades
    mid = (bid + ask) / 2
    ...
    for o in orders:
        if o.side == "buy" and ask <= o.price:
            ...
            hed.cost = COMMISSION; hed.filled = time.time(); hed.pnl = -hed.cost
            trades.append(hed)
            daily_trade_count += 1
            total_trades += 1
...
def simulate_from_keyframes(keyframes, init_bal=10000.0, spacing=100, tick_scale=1440):
    ...
    avg_tp_pnl = round(sum(h.get("pnl", 0) for h in tp_hits) / len(tp_hits), 2) if tp_hits else 0
